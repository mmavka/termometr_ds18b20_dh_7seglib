...
uint8_t One_Wire_Reset(void)								
{
  ...
  { (DDRD &= ~(1 << (6))); (PORTD &= ~(1 << (6))); };                      //отпустить шину
  if ((!(PIND & (1 << (6)))))
  {                                       //Проверяем шину на КЗ	
    __restore_interrupt(intState);        //Restore interrupts.
    return 0x02;
  }		
  { { (DDRD |= (1 << (6))); ; }; (PORTD &= ~(1 << (6))); };                  //Потянуть шину к земле
  __delay_cycles((490)* ((float)(8000000L) / 0x000F4240));		  //ждать 480 микросекунд		
  { (DDRD &= ~(1 << (6))); (PORTD &= ~(1 << (6))); };                      //отпустить шину
  __delay_cycles((70)* ((float)(8000000L) / 0x000F4240));	          //ждать 70 микросекунд
  if ((!(PIND & (1 << (6))))) 
    tmp=0x00;		  //прочитать шину
  else 
    tmp=0x01;
  __delay_cycles((410)* ((float)(8000000L) / 0x000F4240));	          //дождаться окончания инициализации
...
void One_Wire_Write_Bit (uint8_t Bit)
{
  ...
  { { (DDRD |= (1 << (6))); ; }; (PORTD &= ~(1 << (6))); };                  //Потянуть шину к земле	
  __delay_cycles((1)* ((float)(8000000L) / 0x000F4240));	          //ждать 1 микросекунду
  if(Bit) 
    { (DDRD &= ~(1 << (6))); (PORTD &= ~(1 << (6))); };                    //если передаем 1, то отпускаем шину
  __delay_cycles((59)* ((float)(8000000L) / 0x000F4240));	  //задержка 59 микросекунд
  { (DDRD &= ~(1 << (6))); (PORTD &= ~(1 << (6))); };                      //отпустить шину
...
uint8_t One_Wire_Read_Bit (void)
{
  ...
  { { (DDRD |= (1 << (6))); ; }; (PORTD &= ~(1 << (6))); };                        //Потянуть шину к земле	
  __delay_cycles((1)* ((float)(8000000L) / 0x000F4240));
  { (DDRD &= ~(1 << (6))); (PORTD &= ~(1 << (6))); };                            //отпустить шину
  __delay_cycles((10)* ((float)(8000000L) / 0x000F4240));	        //задержка 15 микросекунд
  result = (PIND & (1 << (6)));                 //прочитать шину
  __delay_cycles((45)* ((float)(8000000L) / 0x000F4240)); 	//оставшееся время
