...
uint8_t DS18B20_Start_Conversion_by_ROM (uint8_t (*Serial_Num)[0x08]);
uint8_t DS18B20_Search_Rom (uint8_t *devices_found, uint8_t (*SN_ROM)[2][0x08]);
uint8_t DS18B20_Get_Conversion_Result_by_ROM_CRC (uint8_t (*Serial_Num)[0x08], uint16_t * temp_code);
...
//-------------------------Global variables-------------------------------------
int16_t temper[2];                               // температура с датчика
uint8_t rom_codes[2][0x08];   // адреса датчиков
...
//-------------------------Interrupt--------------------------------------------
//RTOS Interrupt
_Pragma("vector = (0x06)") __interrupt void RTOS_ISR_func(void)
{
TimerService();
}
//------------------------------------------------------------------------------
...
void Get_Result_ds18b20 (void)
{
  ...
  if( result == 0x00 ) {            //если ошибок нет
