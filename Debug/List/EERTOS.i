...
volatile static TPTR	TaskQueue[10 + 1];	        // очередь указателей
volatile static struct {TPTR GoToTask; 			        // Указатель перехода
                        uint16_t Time;			        // Выдержка в мс
                       } MainTimer[5 + 1];     // Очередь таймеров
...
inline void InitRTOS(void)
{
  ...
  for (index = 0; index != 10 + 1; index++)  // Во все позиции записываем Idle
  {
    TaskQueue[index] = Idle;
  }
  
  for (index = 0; index != 5 + 1; index++)// Обнуляем все таймеры.
  {
...
intState = __save_interrupt();
  ...
  {                                                     // с значением Idle - конец очереди.
    index++;
    if (index == 10 + 1)                     // Если очередь переполнена то выходим не солоно хлебавши
    {
...
void SetTimerTask(TPTR TS, uint16_t NewTime)
{
  ...
  for (index = 0; index != 5 + 1; ++index)// Прочесываем очередь таймеров
  {
  ...
  for (index = 0; index != 5 + 1; ++index)// Если не находим похожий таймер, то ищем любой пустой	
  {
...
inline void TaskManager(void)
{
  ...
  {
    for (index = 0; index != 10; index++)    // В противном случае сдвигаем всю очередь
    {
      TaskQueue[index] = TaskQueue[index + 1];
    }
    TaskQueue[10] = Idle;		        // В последнюю запись пихаем затычку
    __enable_interrupt();			        // Разрешаем прерывания
